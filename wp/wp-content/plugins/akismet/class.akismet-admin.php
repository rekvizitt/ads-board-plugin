...
class Akismet_Admin
{
                ...
                $current_screen->add_help_tab([
                    ...
                    "content" =>
                        ...
                        sprintf(
                            ...
                            '<a href="https://akismet.com/pricing/?utm_source=akismet_plugin&amp;utm_campaign=plugin_static_link&amp;utm_medium=in_plugin&amp;utm_content=help_signup" target="_blank">Akismet.com</a>',
                        ) .
                        "</p>",
                ]);
    ...
    public static function enter_api_key()
    {
        ...
        foreach (
            ...
            update_option(
                $option,
                isset($_POST[$option]) && (int) $_POST[$option] == 1
                    ? "1"
                    : "0",
            );
        }
    ...
    public static function recheck_queue()
    {
        ...
        if (
            empty($_POST["nonce"]) ||
            !wp_verify_nonce($_POST["nonce"], "akismet_check_for_spam")
        ) {
            wp_send_json([
                "error" => __(
    ...
    public static function comment_row_action($a, $comment)
    {
        ...
        if (
            !$akismet_result &&
            get_comment_meta($comment->comment_ID, "akismet_skipped", true)
        ) {
        ...
        if ($show_user_comments) {
            ...
            echo sprintf(
                esc_html(
                    _n("%s approved", "%s approved", $comment_count, "akismet"),
                ),
        ...
        if ($history) {
            foreach ($history as $row) {
                ...
                if (!empty($row["message"])) {
                    ...
                    switch ($row["event"]) {
                        ...
                        case "check-ham-pending":
                            $message = esc_html(
                                __(
                                    "Akismet provisionally cleared this comment.",
                                    "akismet",
                                ),
                            );
                            break;
                        ...
                        default:
                            if (
                                preg_match("/^status-changed/", $row["event"])
                            ) {
                ...
                if (!empty($message)) {
                    ...
                    if (isset($row["time"])) {
                        $time =
                            gmdate("D d M Y @ h:i:s a", (int) $row["time"]) .
                            " GMT";
    ...
    public static function get_usage_limit_alert_data()
    {
        return [
            ...
            "upgrade_via_support" =>
                get_option("akismet_alert_upgrade_via_support") === "true",
            "recommended_plan_name" => get_option(
                "akismet_alert_recommended_plan_name",
            ),
        ];
    }
    ...
    public static function display_start_page()
    {
        ...
        $existing_key_is_valid = !(
            self::get_notice_by_key("status") ===
            self::NOTICE_EXISTING_KEY_INVALID
        );
    ...
    public static function display_configuration_page()
    {
        ...
        if (empty(self::$notices)) {
            if (
                ...
                $akismet_user->status == Akismet::USER_STATUS_ACTIVE &&
        ...
        if (
            ...
            in_array($akismet_user->status, [
                Akismet::USER_STATUS_CANCELLED,
                Akismet::USER_STATUS_SUSPENDED,
                Akismet::USER_STATUS_MISSING,
                Akismet::USER_STATUS_NO_SUB,
            ])
        ) {
    ...
    protected static function get_inline_css(): string
    {
        ...
        $inline_css =
            ...
            esc_attr(
                Akismet_Compatible_Plugins::DEFAULT_VISIBLE_PLUGIN_COUNT + 1,
            ) .
            ') {
				display: none;
			}
