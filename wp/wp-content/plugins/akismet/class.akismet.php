...
class Akismet
{
    ...
    private static function init_hooks()
    {
        ...
        add_action(
            "wp_insert_comment",
            ["Akismet", "schedule_email_fallback"],
            10,
            2,
        );
        add_action(
            "wp_insert_comment",
            ["Akismet", "schedule_approval_fallback"],
            10,
            2,
        );
        ...
        add_action(
            ...
            ["Akismet", "email_fallback"],
            10,
            3,
        );
        add_action(
            "akismet_approval_fallback",
            ["Akismet", "approval_fallback"],
            10,
            3,
        );
        ...
        add_filter(
            "notify_moderator",
            ["Akismet", "disable_emails_if_unreachable"],
            1000,
            2,
        );
        add_filter(
        ...
        add_filter("jetpack_contact_form_html", [
            "Akismet",
        ...
        add_filter("gform_akismet_fields", [
            "Akismet",
            "prepare_custom_form_values",
        ]);
        ...
        add_filter("wpcf7_akismet_parameters", [
            "Akismet",
            "prepare_custom_form_values",
        ]);

        // Formidable Forms
        add_filter("frm_filter_final_form", [
            "Akismet",
            "inject_custom_form_fields",
        ]);
        add_filter("frm_akismet_values", [
            "Akismet",
            "prepare_custom_form_values",
        ]);
         ...
         */
        add_filter("fluentform_form_element_start", [
            "Akismet",
        ...
        add_filter("fluentform/form_element_start", [
            "Akismet",
            "output_custom_form_fields",
        ]);
        add_filter(
    ...
    public static function auto_check_comment(
        ...
        if (!is_null($post)) {
            ...
            $category_names = wp_get_post_categories($post->ID, [
                "fields" => "names",
            ]);
        ...
        $commentdata["comment_meta"]["akismet_as_submitted"] =
            $commentdata["comment_as_submitted"];
    ...
    {
        self::log(
            "Checking whether to schedule_email_fallback for comment #" . $id,
        );

        // If the moderation/notification emails for this comment were delayed
        $email_delay = get_comment_meta(
            $id,
            "akismet_schedule_email_fallback",
            true,
        );
    ...
    public static function email_fallback($comment_id)
    {
        ...
        if (
            ...
            self::log(
                "No need to send fallback email for comment #" . $comment_id,
            );
        }
    ...
    {
        self::log(
            "Checking whether to schedule_approval_fallback for comment #" .
                $id,
        );

        // If the moderation/notification emails for this comment were delayed
        $approval_delay = get_comment_meta(
            $id,
            "akismet_schedule_approval_fallback",
            true,
        );
        ...
        if ($approval_delay) {
            ...
            self::log(
                "No need to schedule_approval_fallback for comment #" . $id,
            );
        }
    }
    ...
    public static function approval_fallback($comment_id)
    {
        ...
        if (wp_get_comment_status($comment_id) == "unapproved") {
            if (
                self::last_comment_status_change_came_from_akismet($comment_id)
            ) {
    ...
    public static function get_user_comments_approved(
         ...
         * @since 4.2.2
         */
        $excluded_comment_types = apply_filters(
        ...
        if (
            is_array($excluded_comment_types) &&
            !empty($excluded_comment_types)
        ) {
    ...
    public static function submit_spam_comment($comment_id)
    {
        ...
        $comment["comment_check_response"] = self::last_comment_check_response(
            $comment_id,
        );
    ...
    private static function comments_match($comment1, $comment2)
    {
        ...
        if (
            !empty($comment1["akismet_guid"]) &&
            !empty($comment2["akismet_guid"])
        ) {
    ...
    {
        return self::matches_last_comment(
            self::get_fields_for_comment_matching($comment_id),
        );
    }
     ...
     */
    public static function disable_emails_if_unreachable(
        $maybe_notify,
        $comment_id,
    ) {
        if ($maybe_notify) {
            if (
                ...
                self::log(
                    "Disabling notification email for comment #" . $comment_id,
                );
    ...
    public static function update_alert($response)
    {
        ...
        foreach ($alert_header_names as $alert_header_name) {
            $value = null;
            if (
                isset($response[0][$alert_header_prefix . $alert_header_name])
            ) {
                $value =
                    $response[0][$alert_header_prefix . $alert_header_name];
            }

            $option_name =
                $alert_option_prefix .
                str_replace("-", "_", $alert_header_name);
    ...
    public static function inject_custom_form_fields($html)
    {
        $html = str_replace(
            "</form>",
            self::get_akismet_form_fields() . "</form>",
            $html,
        );
...
<p><?php echo esc_html($message); ?></p>
  ...
  exit();
     ...
     */
    public static function pre_check_pingback(
        $method,
        $args = [],
        $server = null,
    ) {
    ...
    public static function load_form_js()
    {
        if (
            ...
            wp_register_script(
                ...
                [],
                filemtime(
                    plugin_dir_path(__FILE__) . "_inc/akismet-frontend.js",
                ),
                true,
            );
     ...
     */
    public static function load_form_js_via_filter(
        $return_value,
        $tag,
        $attr,
        $m,
     ...
     * @return bool
     */
    public static function last_comment_status_change_came_from_akismet(
        $comment_id,
    ) {
        $history = self::get_comment_history($comment_id);
        ...
        if (
            in_array(
                $most_recent_history_event["event"],
                $akismet_history_events,
            )
        ) {
            return true;
