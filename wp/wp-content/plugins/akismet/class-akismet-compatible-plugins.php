...
class Akismet_Compatible_Plugins
{
     ...
     * @var array
     */
    protected const COMPATIBLE_PLUGIN_FIELDS = [
        "slug",
        "name",
        "logo",
        "help_url",
    ...
    public static function get_installed_compatible_plugins(
        ...
        foreach ($compatible_plugins as $slug => $data) {
            ...
            $network_active =
                is_multisite() && is_plugin_active_for_network($path);
    ...
    public static function init(): void
    {
        add_action(
            "activated_plugin",
            [static::class, "handle_plugin_change"],
            true,
        );
        add_action(
            "deactivated_plugin",
     ...
     * @return array
     */
    private static function get_compatible_plugins(
        bool $bypass_cache = false,
    ): array {
    ...
    private static function validate_compatible_plugin_response($response)
    {
        ...
        foreach ($plugins as $plugin) {
            ...
            $plugin_key_count = count(
                array_intersect_key(
                    $plugin,
                    array_flip(static::COMPATIBLE_PLUGIN_FIELDS),
                ),
            );
    ...
    {
        return preg_match('/^[a-zA-Z0-9._-]+\/[a-zA-Z0-9_-]+\.php$/', $path) ===
            1;
    }
    ...
    private static function sanitize_compatible_plugin_response(
        ...
        foreach ($plugins as $key => $plugin) {
            ...
            $plugins[$key]["help_url"] = sanitize_url(
                $plugins[$key]["help_url"],
            );
    ...
    private static function get_cached_plugins()
    {
        ...
        return get_transient(static::CACHE_KEY . "_$_blog_id");
    }
    ...
    private static function purge_cache(): bool
    {
        ...
        return delete_transient(static::CACHE_KEY . "_$_blog_id");
    }
}
